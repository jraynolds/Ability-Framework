...
@export var stat : StatResource ## The Stat this Effect adds to.
	...
	match entity_target :
		Targeting.Target.Targets:
			...
			for target in targets:
				target.stats_component.add_stat_value(stat, addition)
		Targeting.Target.Target:
			assert(targets[0], "There is no valid target")
			targets[0].stats_component.add_stat_value(stat, addition)
		Targeting.Target.Caster:
			caster.stats_component.add_stat_value(stat, addition)
